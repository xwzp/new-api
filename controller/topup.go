...
func GetTopUpInfo(c *gin.Context) {
	...
	amountOptions := make([]int64, 0)
	amountDiscount := make(map[int64]float64)
	if tiers, err := model.GetEnabledTopupTiers(); err == nil {
		for _, tier := range tiers {
			amountOptions = append(amountOptions, tier.Amount)
			if tier.Discount > 0 {
				amountDiscount[tier.Amount] = tier.Discount
			}
		}
	}
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if !model.IsTopUpPaymentProvider(topUp, model.PaymentProviderEpay) {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 回调订单支付网关不匹配 trade_no=%s order_provider=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.PaymentProvider, verifyInfo.Type, c.ClientIP()))
			return
		}
		if topUp.Status == common.TopUpStatusPending {
			if topUp.PaymentMethod != verifyInfo.Type {
				logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 实际支付方式与订单不同，更新记录 trade_no=%s order_payment_method=%s actual_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, topUp.PaymentMethod, verifyInfo.Type, c.ClientIP()))
