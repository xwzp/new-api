...
func topUpPaymentProviderMatches(topUp *TopUp, expectedPaymentProvider string) bool {
	...
	case PaymentProviderWaffoPancake:
		return topUp.PaymentMethod == PaymentMethodWaffoPancake && strings.HasPrefix(topUp.TradeNo, "WAFFO_PANCAKE-")
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%.2f", logger.FormatQuota(int(quota)), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodStripe)
...
func RechargeWaffoPancake(tradeNo string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if !topUpPaymentProviderMatches(topUp, PaymentProviderWaffoPancake) {
			return ErrPaymentProviderMismatch
		}
